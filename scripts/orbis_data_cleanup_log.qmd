...
Give the time structure it makes sense to limit the sample to the period 2006 (first year with reasonably stable coverage) to 2021 (last year with reasonably stable coverage). Let's look at the descriptive statistics of our data next.
...
Total assets seems reasonably well-behaved. It is strictly positive but highly left-skewed. That means that there are many small firms in the data but some very large, exactly like you would expect.
